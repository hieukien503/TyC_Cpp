...
static thread_local
#endif
std::unique_ptr<TyCParserStaticData> tycParserStaticData = nullptr;
...
void tycParserInitialize() {
  ...
  tycParserStaticData = std::move(staticData);
}
...
std::any TyCParser::ProgramContext::accept(tree::ParseTreeVisitor *visitor) {
  if (auto parserVisitor = dynamic_cast<TyCVisitor*>(visitor))
